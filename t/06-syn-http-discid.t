...
# 5-15) Test http discid synopsis
&test_syn();
...
sub test_syn {
    SKIP: {
        # Check for skip
        if ( ! $::opt{test_http_discid_syn} ) {
            my $fn = File::Spec->catfile( 't', 'options' );
            my_skip( "test_http_discid_syn is not set in file '$fn'" );
        }

        # 2) Require synopsis
        my $fn = File::Spec->catfile( 'gen', 'syn-http-discid.pl' );
        try {
            require( $fn );
        }
